...
void init()
{
  ...
  // Set up the state machine for encoders 1-4
  pio = pio0;
  ...
  // Set up the state machine for encoder 5
  pio_1 = pio1;
  enc_val[4], prev_enc_val[4], cur_enc_val[4] = 0;
  encoders_program_init(pio_1, 3, offset, ENC_GPIO[4], ENC_DEBOUNCE);

  dma_channel_config c = dma_channel_get_default_config(3);
  channel_config_set_read_increment(&c, false);
  channel_config_set_write_increment(&c, false);
  channel_config_set_dreq(&c, pio_get_dreq(pio_1, 3, false));

  dma_channel_configure(3, &c,
                        &enc_val[4],  // Destination pointer
                        &pio_1->rxf[3], // Source pointer
                        0x10,         // Number of transfers
                        true          // Start immediately
  );
